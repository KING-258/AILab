def water_jug_dfs(jug1_capacity, jug2_capacity, target, jug1_amount=0, jug2_amount=0, visited=None, path=None):
    if visited is None:
        visited = set()
    if path is None:
        path = []
    ...
    if jug1_amount < jug1_capacity:
        ...
        if next_state not in visited:
            new_path = water_jug_dfs(jug1_capacity, jug2_capacity, target, *next_state, visited, path + [current_state])
            if new_path:
                return new_path
...
result = water_jug_dfs(jug1_capacity, jug2_capacity, target_amount)
if result:
